...
## 2026-08-31 · feat: move daily brief to 05:00 and 17:00 China time

- 提交：`feat: move daily brief to 05:00 and 17:00 China time`（提交前记录）
- 栏目/模块：每日邮报调度、编辑上下文、历史板块质量闸门、订阅回复、部署文档
- 更新：将 Asia/Shanghai 下的两次日报生成时间调整为 05:00 和 17:00，订阅预检调整为 04:45 和 16:45；同步早间/晚间版市场状态、历史上的今天规则、去重上下文、订阅成功提示、systemd 模板及使用文档。
- 验证：本地 67 个 Python 回归测试、compileall、Bash 语法和隐私扫描通过；生产 VPS 已完成备份、精确原子替换、systemd 日历/单位校验、timer 重载、下一次调度和代码语法/版次行为检查；未手动补发邮件，真实收件结果仍待下一次自动运行验收。
