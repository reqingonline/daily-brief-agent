...
- 每次生成先读取上一封邮报的完整可见正文，并结合最近几封简报减少跨期重复；
- 11:00 版保留“历史上的今天”，23:00 版继续发送但省略该板块；事实核查无合适内容时整段省略；
...
- 邮报标题区域之后由运行器注入生成模型和本封邮件完整生成流程的 token 总量，初稿与自动修复一并统计；
...
两版每次生成前都会读取上一封邮报作为硬去重基线。11:00 版以不重复的“历史上的今天”收尾；23:00 版不生成该板块。事实核查只有在存在高传播、可能误导且影响较大的说法时出现。

11:00/23:00 是生成卡点，不是保证收件的时刻。实际耗时主要来自联网核验和模型推理，SMTP 通常只占很小一部分。

通过质量校验后，运行器会从 Codex 的结构化 JSONL 完成事件中汇总本期所有模型调用的 token 总量，并在标题区域之后增加一行浅色注释。该注释由程序写入，不由模型自行填写；如果当前 CLI 没有返回可靠用量，邮件会明确显示“暂不可得”，不会用估算值替代。原始事件只在本次任务的私有临时工作区中短暂存在，不归档、不提交。
